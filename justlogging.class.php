...
class Justlogging {
  ...
  function alert($entry, $log_key = null){
    $this->log($entry, $log_key, true);
  }
  
  function log($entry, $log_key = null, $alert = false){
    ...
    $post_data = ($alert == true) ? $post_data . "&alert=true" : $post_data;

    $ch = curl_init(); 
    curl_setopt($ch, CURLOPT_URL, 'http://logs.justlogging.com/log'); // set url to post to 
